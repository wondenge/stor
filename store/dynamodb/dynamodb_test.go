...
const TestTableName = "test-1-stor"
...
func TestRegister(t *testing.T) {
	...
	kv, err := stor.NewStore(
		store.DYNAMODB,
		[]string{},
		&store.Config{Bucket: "test-1-stor"},
	)
...
func TestDynamoDBStore(t *testing.T) {
	...
	stor.RunTestCommon(t, ddbStore)
	stor.RunTestAtomic(t, ddbStore)
	stor.RunTestTTL(t, ddbStore, backupStore)
}

func TestDynamoDBStoreLock(t *testing.T) {
	ddbStore := newDynamoDBStore(t)
	backupStore := newDynamoDBStore(t)
	stor.RunTestLock(t, ddbStore)
	stor.RunTestLockTTL(t, ddbStore, backupStore)
}
